...
function startStorageServers {
    ...
    ${STORAGE_SERVER} -N=${N} -vids=${STORAGE_ID[0]} -port=${STORAGE_PORT} &> /dev/null &
    ...
    then
	    ...
	    STORAGE_SLAVE_PORT=$(((RANDOM % 10000) + 10000))
            ${STORAGE_SERVER} -port=${STORAGE_SLAVE_PORT} -vids=${STORAGE_ID[$i]} -master="localhost:${STORAGE_PORT}" &> /dev/null &
...
function testStressSingleClientSingleTribMultipleStorage {
    ...
    STORAGE_ID=('700000000,2100000000' '1500000000,2600000000,3200000000' '3800000000')
    M=1
    CLIENT_COUNT=('1')
    TIMEOUT=15
    testStress
...
function testStressMultipleClientSingleTribMultipleStorage {
    ...
    STORAGE_ID=('500000000' '1000000000,2500000000' '1700000000,3000000000,3700000000'
    '750000000' '1300000000' '4200000000')
    M=1
    CLIENT_COUNT=('1' '1' '1')
    TIMEOUT=15
    testStress
...
function testStressMultipleClientMultipleTribMultipleStorage {
    ...
    STORAGE_ID=('500000000' '1000000000,2500000000' '1700000000,3000000000,3700000000'
    '750000000' '1300000000' '4200000000,3300000000' '250000000')
    M=3
    CLIENT_COUNT=('1' '1' '1')
    TIMEOUT=30
    testStress
