...
func init() {
	...
	flag.Usage = func() {
		...
		fmt.Fprintln(os.Stderr, "  GetFriends:          fl userID")
...
func main() {
	...
	case "fl": // friends list
		friends, status, err := client.GetFriends(flag.Arg(1))
		printStatus(ci.funcname, status, err)
	...
	case "tp": // tribble post
		_, status, err := client.PostTribble(flag.Arg(1), flag.Arg(2))
		printStatus(ci.funcname, status, err)
	}
}
