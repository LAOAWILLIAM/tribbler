...
func (tc *tribClient) GetFriends(userID string) ([]string, tribrpc.Status, error) {
	args := &tribrpc.GetFriendsArgs{UserID: userID}
	var reply tribrpc.GetFriendsReply
	if err := tc.client.Call("TribServer.GetFriends", args, &reply); err != nil {
		return nil, 0, err
	}
...
func (tc *tribClient) PostTribble(userID, contents string) (string, tribrpc.Status, error) {
	...
	return reply.PostKey, reply.Status, nil
}

func (tc *tribClient) DeleteTribble(userID, postKey string) (tribrpc.Status, error) {
	args := &tribrpc.DeleteTribbleArgs{UserID: userID, PostKey: postKey}
	var reply tribrpc.DeleteTribbleReply
	if err := tc.client.Call("TribServer.DeleteTribble", args, &reply); err != nil {
		return 0, err
	}
	return reply.Status, nil
}
