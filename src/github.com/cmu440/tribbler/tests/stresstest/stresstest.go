...
func main() {
	...
	for _, cmd := range cmds {
		switch cmd {
		case GetFriends:
			friends, status, err := client.GetFriends(user)
			if err != nil {
				LOGE.Fatalf("FAIL: GetFriends returned error '%s'\n", err)
			}
			if status == 0 || status == tribrpc.NoSuchUser {
				LOGE.Fatalf("FAIL: GetFriends returned error status '%s'\n", statusMap[status])
			}
			if !validateFriends(&friends) {
				LOGE.Fatalln("FAIL: failed while validating returned friends")
			}
		case AddSubscription:
...
func validateFriends(friends *[]string) bool {
	friendSet := make(map[string]bool, len(*friends))
	for _, friend := range *friends {
		if friendSet[friend] == true {
			return false
		}
